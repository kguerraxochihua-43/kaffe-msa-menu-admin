...
class ProductCreationPostgresTest {
    ...
    ProductCreationRequests requests;
    ...
    long create(long tenant, long actor, Map<String,Object> body) {
        return tx.execute(status -> {
            var claim = requests.begin(tenant, actor, body);
            if (claim.productId() != null) return claim.productId();
    ...
    @Test void concurrentAndLostResponseRetriesCreateOnlyOneProduct() throws Exception {
        ...
        assertThat(jdbc.queryForObject("select count(*) from menu.product_creation_requests",Long.class)).isEqualTo(1);
    }
    ...
    @Test void rollbackDoesNotConsumeTheKey() {
        ...
        assertThatThrownBy(() -> tx.execute(status -> {
            var claim = requests.begin(1,1,body);
    ...
    @Test void invalidKeyFailsBeforeCreationAndLegacyClientsRemainCompatible() {
        ...
        create(1,1,body);
        assertThat(jdbc.queryForObject("select count(*) from menu.product_creation_requests",Long.class)).isZero();
    }
}
